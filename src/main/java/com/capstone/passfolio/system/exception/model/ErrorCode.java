...
public enum ErrorCode {
    ...
    AI_BAD_INPUT(HttpStatus.BAD_REQUEST, "AI_BAD_INPUT", "AI 서버가 입력 데이터를 거부했습니다."),
    ...
    ANALYSIS_PORTFOLIO_INPUT_REQUIRED(HttpStatus.BAD_REQUEST, "ANALYSIS_PORTFOLIO_INPUT_REQUIRED", "NONSTOP 모드는 포트폴리오 파일과 생성 방식(EDIT/GENERATE)이 필요합니다."),
    ANALYSIS_PORTFOLIO_FILE_TYPE_MISMATCH(HttpStatus.BAD_REQUEST, "ANALYSIS_PORTFOLIO_FILE_TYPE_MISMATCH", "선택한 포트폴리오 생성 방식과 업로드한 문서 종류가 일치하지 않습니다."),
    ANALYSIS_PORTFOLIO_HANDOFF_NOT_RETRYABLE(HttpStatus.CONFLICT, "ANALYSIS_PORTFOLIO_HANDOFF_NOT_RETRYABLE", "포트폴리오 생성을 재시도할 수 없는 배치입니다."),
